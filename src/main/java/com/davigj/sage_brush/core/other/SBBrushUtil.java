...
//            tamable.lookAt(player, 30.0F, 30.0F);
        ...
        if (victim instanceof Turtle turtle && SBConfig.COMMON.scute.get()) {
            if (level.isClientSide) {
                ...
                if (timer == 0) {
                    turtle.spawnAtLocation((ServerLevel) level, Items.TURTLE_SCUTE);
                    damageItem(stack, player);
        ...
        if (victim instanceof Armadillo armadillo) {
            if (level.isClientSide) {
                ...
                if (timer == 0) {
                    armadillo.spawnAtLocation((ServerLevel) level, Items.ARMADILLO_SCUTE);
                    damageItem(stack, player);
        ...
        if (SBConfig.COMMON.torScute.get() && (FabricLoader.getInstance().isModLoaded("sullysmod") && SBConstants.isTortoise(victim))) {
            if (level.isClientSide) {
                ...
                if (timer == 0) {
                    victim.spawnAtLocation((ServerLevel) level, SBConstants.tortoiseScute);
                    damageItem(stack, player);
    ...
    private static void pluck(ItemStack stack, LivingEntity player, Entity pluckee, AttachmentType<Integer> timerData, int reset) {
        ...
        if (timer == 0) {
            ...
            if (pluckee.level() instanceof ServerLevel serverLevel)
                pluckee.spawnAtLocation(serverLevel, item);
            damageItem(stack, player);
